class halfAlgorithm:
    ...
    def decide(self, macdDayBefore, macdToday, signalDayBefore, signalToday, price):
        ...
        elif macdDayBefore > signalDayBefore and macdToday <= signalToday and self.numberOfActions > 1:
            #SELLING
            actionsToSell = round(self.numberOfActions/2, 0)
            self.money += actionsToSell * price
            self.money = round(self.money, 2)
            self.numberOfActions -= actionsToSell
        return
